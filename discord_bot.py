...
class _GeminiVoiceConversation:
	def __init__(self, client: discord.Client, command_prefix: str = ">") -> None:
		...
		self._genai_client = genai.Client(api_key=get_paid_gemini_api_key(), http_options={"api_version": "v1alpha"})
...
class DiscordBotWrapper:
	def __init__(
		...
		if self.voice_available and VOICE_RECV_AVAILABLE and get_paid_gemini_api_key():
	...
	def _register_events(self) -> None:
		...
		async def on_ready() -> None:
			...
			if not get_paid_gemini_api_key():
				logging.warning("PAID_GEMINI_API_KEY is not set: Gemini Live voice conversation is disabled.")
